...
export default class MainApp extends React.Component {
  ...
  componentDidMount = () => {
    this.drive.getFolder((folder_id) => {
      if (!folder_id) {
        this.setState({ toast: 'App does not have permission to access drive...please check google-account settings Signing out...' });
        setTimeout(() => {
  ...
  onPasswordLoaded = (loading) => {
    if (loading) {
      this.setState({ action: null, message: 'Setting Everything Up....', email: null });
    } else {
